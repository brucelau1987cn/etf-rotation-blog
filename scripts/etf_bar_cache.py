...
def get_bars(db: sqlite3.Connection, market: str, symbol: str, adjustment: str = "qfq", limit: int = 90) -> list[dict[str, Any]]:
    ...
    priority = "CASE source WHEN 'tencent' THEN 1 WHEN 'cf-baostock' THEN 2 WHEN 'local-baostock' THEN 3 WHEN 'baostock' THEN 3 WHEN 'iwencai' THEN 4 WHEN 'stock-api' THEN 5 ELSE 9 END"
    rows = db.execute(
        f"""SELECT * FROM (
