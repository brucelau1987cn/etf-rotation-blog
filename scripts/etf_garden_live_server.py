...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_HEAD(self):
        ...
        if url.path == "/health":
            ...
            self.end_headers()
        elif url.path in {"/api/etf-garden/live", "/api/us-etf-garden/live"}:
            self.send_response(200)
            self.send_header("Content-Type", "application/json; charset=utf-8")
            self.send_header("Access-Control-Allow-Origin", "*")
    ...
    def do_GET(self):
        ...
        try:
            if url.path == "/health":
                ...
                self._send_json(200, {"ok": True, "service": "etf-garden-live", "version": "1.4", "uptime_s": round(time.time() - SERVER_START), "cache_ttl_s": CACHE_TTL, "cache_age_s": round(cache_age, 2) if cache_age is not None else None})
