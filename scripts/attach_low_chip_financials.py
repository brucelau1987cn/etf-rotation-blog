...
def main() -> None:
    ...
    for code in payload["intersection"]:
        row = by_code[code]
        roe = field(row, "净资产收益率roe")
        net_margin = field(row, "销售净利率")
        cash_flow = field(row, "经营活动产生的现金流量净额")
        net_profit = field(row, "归属于母公司所有者的净利润")
        gross_margin = field(row, "销售毛利率")
        debt_ratio = field(row, "资产负债率")
        cash_profit_ratio = (
