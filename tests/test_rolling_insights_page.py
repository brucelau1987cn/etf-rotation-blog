...
def test_latest_daily_report_contract_and_archive():
    ...
    assert "rollingDailyReports['2026-09-10']" in page
    assert "'2026-09-10': {" in data
    latest = data[data.index("'2026-09-10': {"):data.index("'2026-09-09': {")]
    assert latest.count("name: '") == 6
    assert latest.count("validation: 'confirmed'") == 1
    assert latest.count("validation: 'watch'") == 5
    for text in ("白银现货", "东方明珠", "国民技术", "三安光电", "中国宏桥", "澜起科技", "今日操作结论", "什么时候买", "什么时候卖", "今日信号表", "逐标的计划", "执行纪律"):
        assert text in latest + component
    assert (ROOT / "src/pages/rolling/insights/2026-09-09.astro").exists()
    assert "/rolling/insights/2026-09-10 /rolling/insights/ 301" in redirects
