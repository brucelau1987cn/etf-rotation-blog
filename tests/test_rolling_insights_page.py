...
def test_latest_daily_report_contract_and_archive():
    ...
    assert "rollingDailyReports['2026-09-22']" in page
    assert "'2026-09-22': {" in data
    latest = data[data.index("'2026-09-22': {"):data.index("'2026-09-21': {")]
    assert latest.count('{ name:') == 5
    assert latest.count("validation: 'confirmed'") == 1
    assert latest.count("validation: 'watch'") == 4
    for text in ("创新医疗", "海光信息", "中国宏桥", "纽约白银期货", "今日操作结论", "什么时候买", "什么时候卖", "今日信号表", "逐标的计划", "执行纪律"):
...
def test_insights_2026_09_22():
    import re
    p = ROOT / 'src/data/rolling-daily-insights.ts'
    data = p.read_text(encoding='utf-8')
    assert "'2026-09-22':" in data, 'today report key missing'
    ...
    assert data.index("'2026-09-22':") < data.index("'2026-09-21':"), 'today must come before prev'
    page = (ROOT / 'src/pages/rolling/insights.astro').read_text(encoding='utf-8')
    assert "rollingDailyReports['2026-09-22']" in page, 'insights.astro not pointing to today'
    cat = re.search(r"rollingDailyArticleCatalog = \[(.+?)\];", data, re.S).group(1)
    assert "'2026-09-22', href: '/rolling/insights/'" in cat, 'catalog latest must be today at /rolling/insights/'
    assert "'2026-09-21', href: '/rolling/insights/2026-09-21/'" in cat, 'catalog demote entry missing'
    assert (ROOT / 'src/pages/rolling/insights/2026-09-21.astro').exists(), 'prev static page missing'
    redirects = (ROOT / 'public/_redirects').read_text(encoding='utf-8')
