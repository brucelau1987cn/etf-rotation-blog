...
(function() {
  ...
  const isSellObservation = (code) => code === '5m';
  ...
  const normalizeTimeline = (data) => {
    ...
    // 多方重置空方历史：空方信号只保留最近一次正式多方之后的窗口
    // （原则 多方→空方→多方→空方；新多方出现后，更早的空方不再显示）。
    const latestBuyTime = allBuys.reduce((max, item) => Math.max(max, signalTimeMs(item)), 0);
    const roundSells = latestBuyTime > 0
      ? timeline.filter(item => item && item.type === 'SELL' && signalTimeMs(item) >= latestBuyTime)
      : timeline.filter(item => item && item.type === 'SELL');
    const sellObservation = roundSells.find(item => item && isSellObservation(item.code)) || null;
    const allSells = roundSells.filter(item => item && !isSellObservation(item.code));
  ...
  const updateBoard = (symbol, data) => {
    ...
    setWatchDot(
      ...
      !!sellObservation,
      `5m 观察已触发 ${formatTime(sellObservation?.triggered_at, true, false)}`,
      '5m 观察窗口未触发'
    );
  };
