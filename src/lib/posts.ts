...
import { renameCompassTerms, sanitizePublicText } from './sanitizePublicText.mjs';

export type BlogPost = CollectionEntry<'blog'>;

export { renameCompassTerms, sanitizePublicText };
export { stripToolNames } from './sanitizePublicText.mjs';
...
export const isCompassSystemPost = (post: BlogPost) => post.id.startsWith('a-compass/');
...
export const isPublicEtfPost = (post: BlogPost) => !isStockPost(post) && !isCompassSystemPost(post);
...
export const cleanDescription = (text: string) => sanitizePublicText(text)
