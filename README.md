...
- **滚动多空信号**：
  ...
  - 每行右侧提供 TradingView 技术分析；A股/美股/期货使用紧凑组件，港股显示 TradingView 4位代码及4小时/1天/1周抓取评级；港股交易时段每60秒刷新，休市与后台标签页暂停；窄屏整行展开，分页始终只加载当前 3 只
...
| 低筹码股 | 仅股票页 `/rolling/low-chip/`，内部模型结果 + 日归档 |
